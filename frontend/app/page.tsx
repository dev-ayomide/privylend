...
export default function Home() {
  ...
  const activeLoansCount = displayLoans.filter(loan => loan.status === 'Active' || loan.status === 'MarginCall').length;
  const totalBorrowed = displayLoans.filter(loan => loan.status === 'Active' || loan.status === 'MarginCall')
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="text-center space-y-4 py-8">
        ...
        <p className="text-lg text-slate-600 max-w-2xl mx-auto">
          Privacy-preserving lending protocol built on Canton. Deposit cBTC, cETH, USDC, or USDT as collateral and borrow with confidence.
        </p>
        {error && !USE_MOCK_DATA && (
          <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg max-w-2xl mx-auto">
            <p className="text-sm text-yellow-800">
              Unable to connect to Canton Network. Using demo mode.
            </p>
          </div>
        )}
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-500">Effective value across {displayCollateral.length} accounts</p>
          </CardContent>
        </Card>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-slate-600">Manage and monitor your deposited Canton-native assets</p>
        </div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {displayCollateral.length === 0 ? (
            <div className="col-span-full text-center py-8">
              <p className="text-slate-600">No collateral accounts found.</p>
              <p className="text-sm text-slate-500 mt-2">Deposit cBTC, cETH, USDC, or USDT to get started.</p>
            </div>
          ) : (
      ...
      <Card className="border-slate-200 shadow-sm">
        <CardHeader>
          ...
          <CardDescription>How PrivyLend protects your information on Canton Network</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-3 gap-6">
            <div className="space-y-2 p-4 rounded-lg bg-slate-50 border border-slate-200">
              ...
              <p className="text-sm text-slate-600 leading-relaxed">
                Only you and the lending pool see your collateral and loan details on Canton
              </p>
            </div>
            <div className="space-y-2 p-4 rounded-lg bg-slate-50 border border-slate-200">
              <h3 className="font-semibold text-slate-900">Risk Monitoring</h3>
              <p className="text-sm text-slate-600 leading-relaxed">
                Automatic LTV tracking with margin calls at 80% and liquidation at 85%
              </p>
            </div>
            <div className="space-y-2 p-4 rounded-lg bg-slate-50 border border-slate-200">
              <h3 className="font-semibold text-slate-900">Canton-Native</h3>
              <p className="text-sm text-slate-600 leading-relaxed">
                cBTC, cETH, USDC, USDT - all assets are native to Canton Network
              </p>
            </div>
          </div>
        </CardContent>
